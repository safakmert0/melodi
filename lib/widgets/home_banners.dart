...
class HomeBanners extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [_MetadataBackfillBanner(md: md), _LyricsBackfillBanner(md: md), const _WaitingForLosslessBanner(), _FailedDownloadsBanner(dp: dp), const _LibraryHealthBanner()]);
  }
}
...
class _MetadataBackfillBannerState extends State<_MetadataBackfillBanner> {
  ...
  Widget build(BuildContext context) {
    ...
    return _BannerCard(type: BannerType.info, icon: Icons.image_rounded, message: AppLocale.tr('x_tracks_need_art').replaceAll('{count}', '$_trackCount'), actionLabel: 'Start', onAction: () {widget.md.startBackfillAlbumArt(); _dismiss();}, onDismiss: _dismiss);
  }
...
class _LyricsBackfillBannerState extends State<_LyricsBackfillBanner> {
  ...
  Future<void> _load() async {
    ...
    final tracks = await db.rawQuery('''SELECT COUNT(*) as count FROM songs s WHERE (s.lyrics IS NULL OR s.lyrics = '') AND NOT EXISTS (SELECT 1 FROM track_lyrics tl WHERE tl.trackId = s.id)''');
  ...
  Widget build(BuildContext context) {
    if (_dismissed || _trackCount == 0) return const SizedBox.shrink();
    return _BannerCard(type: BannerType.info, icon: Icons.lyrics_rounded, message: AppLocale.tr('x_tracks_need_lyrics').replaceAll('{count}', '$_trackCount'), actionLabel: 'Start', onAction: () {widget.md.startBackfillLyrics(); _dismiss();}, onDismiss: _dismiss);
  }
...
class _WaitingForLosslessBannerState extends State<_WaitingForLosslessBanner> {
  ...
  Widget build(BuildContext context) {
    ...
    return _BannerCard(type: BannerType.warning, icon: Icons.hourglass_empty_rounded, message: AppLocale.tr('x_tracks_waiting_lossless').replaceAll('{count}', '$_count'), onDismiss: () {setState(() => _dismissed = true); BannerDismiss.dismiss('waiting_lossless');});
  }
}
...
class _FailedDownloadsBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return _BannerCard(type: BannerType.error, icon: Icons.error_outline_rounded, message: AppLocale.tr('x_downloads_failed').replaceAll('{count}', '${dp.failedCount}'), actionLabel: 'View', onAction: () {Navigator.of(context).push(MaterialPageRoute(builder: (_) => const FailedDownloadsScreen()));});
  }
}
...
class _LibraryHealthBannerState extends State<_LibraryHealthBanner> {
  ...
  Widget build(BuildContext context) {
    ...
    return _BannerCard(type: BannerType.warning, icon: Icons.favorite_border_rounded, message: '$_issueCount ${AppLocale.tr('issues_found')}', actionLabel: AppLocale.tr('library_health'), onAction: () {Navigator.of(context).push(MaterialPageRoute(builder: (_) => const LibraryHealthScreen()));}, onDismiss: () => setState(() => _dismissed = true));
  }
}
...
class _BannerCard extends StatelessWidget {
  ...
  const _BannerCard({required this.type, required this.icon, required this.message, this.actionLabel, this.onAction, this.onDismiss});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(8), border: Border.all(color: scheme.outlineVariant.withValues(alpha: 0.5))),
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
      child: Row(
        children: [
          Icon(icon, color: fg, size: 18),
          const SizedBox(width: 10),
          Expanded(child: Text(message, style: Theme.of(context).textTheme.bodySmall?.copyWith(color: fg, fontSize: 12))),
          if (actionLabel != null && onAction != null) ...[
            ...
            InkWell(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(color: scheme.surface, borderRadius: BorderRadius.circular(6), border: Border.all(color: scheme.outlineVariant.withValues(alpha: 0.5))),
                child: Text(actionLabel!, style: Theme.of(context).textTheme.labelSmall?.copyWith(color: scheme.onSurface, fontWeight: FontWeight.w600)),
              ),
            ),
          ],
          if (onDismiss != null) ...[
            const SizedBox(width: 8),
            InkWell(onTap: onDismiss, borderRadius: BorderRadius.circular(12), child: Padding(padding: const EdgeInsets.all(2), child: Icon(Icons.close, color: fg.withValues(alpha: 0.7), size: 16))),
          ],
        ],
      ),
    );
