...
class YouTubeAudioSource extends StreamAudioSource {
  ...
  static final Map<String, _CachedYouTubeStream> _streamCache = {};
  static final Map<String, Future<AudioOnlyStreamInfo>> _inFlight = {};
  ...
  static Future<void> prefetch(
    ...
    try {
      await _resolveShared(videoId.trim(), quality);
    } catch (error) {
      debugPrint('YouTube prefetch failed for $videoId: $error');
    }
  }
  ...
  static Future<AudioOnlyStreamInfo> _resolveShared(
    String videoId,
    String quality, {
    bool forceRefresh = false,
  }) async {
    final key = '$videoId:$quality';
    if (forceRefresh) _invalidate(videoId, quality);
    ...
    final future = _loadStream(videoId, quality);
    _inFlight[key] = future;
    try {
      final stream = await future;
      _streamCache[key] = _CachedYouTubeStream(stream, DateTime.now());
      return stream;
    } finally {
  ...
  static Future<AudioOnlyStreamInfo> _loadStream(
    String videoId,
    String quality,
  ) async {
    ...
    try {
      ...
      return quality == 'high' ? candidates.last : candidates.first;
    } finally {
      youtube.close();
    }
  }

  static void _invalidate(String videoId, String quality) {
    final key = '$videoId:$quality';
...
class _CachedYouTubeStream {
  const _CachedYouTubeStream(this.stream, this.cachedAt);
