...
void main() {
  ...
  group('remote playback duration policy', () {
    test('uses catalogue duration for a much longer remote stream', () {
      final effective = PlaybackDurationPolicy.effectiveDuration(
        expected: const Duration(minutes: 3),
        decoded: const Duration(minutes: 6),
        isRemote: true,
      );

      expect(effective, const Duration(minutes: 3));
      expect(
        PlaybackDurationPolicy.shouldStopAt(
          position: const Duration(minutes: 3),
          effectiveDuration: effective,
          decodedDuration: const Duration(minutes: 6),
        ),
        isTrue,
      );
    });

    test('keeps small decoder differences and all local media durations', () {
      expect(
        PlaybackDurationPolicy.effectiveDuration(
          expected: const Duration(minutes: 3),
          decoded: const Duration(minutes: 3, seconds: 2),
          isRemote: true,
        ),
